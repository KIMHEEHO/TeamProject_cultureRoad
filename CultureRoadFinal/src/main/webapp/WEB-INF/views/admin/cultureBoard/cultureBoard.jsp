...
$(function(){
   ...
   $(".hiddenBtn").click(function(){
      let ctBoNum = $(this).attr("data-num");
      let ctBoHidden = $(this).attr("data-hidden");
      $("#ctBoNum1").val(ctBoNum);
      if(ctBoHidden == 1) {
         $("#ctBoHidden").val(0);
      }else{
         $("#ctBoHidden").val(1);
      ...
      $("#hidden").attr({
         method : "post",
         action : "/admin/culture/boardHidden"
      })
      $("#hidden").submit();
   })
   ...
   $(".goDetail").click(function(){
      ...
      $("#detail").attr({
         method : "get",
         action : "/admin/culture/boardDetail"
      })
      $("#detail").submit();
   })
...
<div class="main-panel">
   <div class="content-wrapper">
      <div class="d-xl-flex justify-content-between align-items-start">
         <h2 class="text-dark font-weight-bold mb-2">문화예술 게시판</h2>
      </div>
      <div class="row">
         <div class="col-md-12">
            <div
...
<!--  위에서부터 여기까지는 꼭 넣어주세요!!!! -->
               <div id="boardSearch" class="d-flex justify-content-end">
                  <form id="f_search" name="f_search">
                     ...
                     <div class="d-inline-flex text-end">
                        ...
                        </select> <input type="text" name="keyword" id="keyword" class="text-right form-control form-control-sm w-auto"
               ...
               <form id="hidden" name="hidden">
                  <input type="hidden" id="ctBoNum1" name="ctBoNum" /> 
                  <input type="hidden" id="ctBoHidden" name="ctBoHidden" /> 
               ...
               <div id="boardList">
                  <table summary="게시판 리스트">
                     <thead id="thead">
                        <tr>
                           <th data-value="ctBoNum">글번호</th>
                           <th>글제목</th>
                           <th>작성자</th>
                           <th data-value="ctBoWriteDate">작성일</th>
                           <th>조회수</th>
                           <th>관리자</th>
                        ...
                        <c:choose>
                           <c:when test="${not empty board}">
                              <c:forEach var="board" items="${board}" varStatus="status">
                                 <tr class="text-center" data-num="${board.ctBoNum}">
                                    <td>${board.ctBoNum}</td>
                                    <td class="goDetail text-start">${board.ctBoTitle}
                                       <c:if test="${board.ctBoReplycnt > 0 }">
                                          <span class="replyCnt"> [${board.ctBoReplycnt}]
                                          </span>
                                       </c:if>
                                    </td>
                                    <td class="name">${board.userName }</td>
                                    <td class="text-center">${board.ctBoWriteDate}</td>
                                    <td class="text-center">${board.ctBoReadcnt}</td>
                                    <td class="text-center"><c:if
                                          test="${board.ctBoHidden == 0 }">
                                          <span data-num="${board.ctBoNum }"
                                             data-hidden="${board.ctBoHidden}"
                                             class="hiddenBtn text-center">숨김</span>
                                       </c:if> <c:if test="${board.ctBoHidden == 1 }">
                                          <span data-num="${board.ctBoNum }"
                                             data-hidden="${board.ctBoHidden}"
