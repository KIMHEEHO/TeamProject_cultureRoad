...
$(function(){
	let word = "<c:out value='${cultureReplyVO.keyword}'/>"; 
	if(word!=""){
		$("#keyword").val('<c:out value='${cultureReplyVO.keyword}'/>');
		$("#keyword").focus();
		$("#search").val('<c:out value='${cultureReplyVO.search}'/>');
	}
	
	$(".hiddenBtn").click(function(){
		let ctReNum = $(this).attr("data-num");
		let ctReHidden = $(this).attr("data-hidden");
		$("#ctReNum1").val(ctReNum);
		if(ctReHidden == 1) {
			$("#ctReHidden").val(0);
		}else{
			$("#ctReHidden").val(1);
		...
		$("#hidden").attr({
			method : "post",
			action : "/admin/culture/replyHidden"
		})
		$("#hidden").submit();
	})
...
<div class="main-panel">
   <div class="content-wrapper">
      <div class="d-xl-flex justify-content-between align-items-start">
         <h2 class="text-dark font-weight-bold mb-2">문화예술 게시판 댓글 관리</h2>
      </div>
      <div class="row">
         <div class="col-md-12">
            <div
...
<!--  위에서부터 여기까지는 꼭 넣어주세요!!!! -->
		...
		<form id="f_search" name="f_search" class="form-inline">
			...
			<div class="d-inline-flex text-end">
				<label></label> <select id="search" name="search"
					...
					<option value="ctReContent">내용</option>
					<option value="ctBoNum">게시글번호</option>
	...
	<br />
		...
		<form id="hidden" name="hidden">
			<input type="hidden" id="ctReNum1" name="ctReNum" />
			<input type="hidden" id="ctReHidden" name="ctReHidden" />
		...
		<div id="boardList" class="table-height noticeboard">
			<table summary="댓글 리스트" >
				<thead id="thead">
					<tr>
						<th data-value="ctReNum" class="order text-center col-md-1" >댓글번호</th>
						<th data-value="ctReNum" class="order text-center col-md-1" >게시글번호</th>
						<th class="text-center col-md-5">내용</th>
						<th class="text-center col-md-1">작성자</th>
						<th data-value="ctReWriteDate" class="order col-md-1 text-center">작성일</th>
					...
					<c:choose>
						<c:when test="${not empty reply}" >
							<c:forEach var="reply" items="${reply}" varStatus="status"> 
								<tr class="text-center" data-num="${reply.ctReNum}">
 									<td>${reply.ctReNum}</td>
 									<td>${reply.ctBoNum}</td>
									<td class="goContent text-start"> ${reply.ctReContent}</td>
									<td class="name">${reply.userName }</td>
									<td class="text-center"> ${reply.ctReWriteDate}</td>
									<td class="text-center">
										<c:if test="${reply.ctReHidden == 0 }">
											<span data-num="${reply.ctReNum }" data-hidden="${reply.ctReHidden}" class="hiddenBtn text-center" >숨김</span>
										</c:if>
										<c:if test="${reply.ctReHidden == 1 }">
											<span data-num="${reply.ctReNum }" data-hidden="${reply.ctReHidden}" class="hiddenBtn text-center" style="color: red;">해제</span>
										</c:if>
									</td>
								</tr>
							</c:forEach>
